...
const DEFAULT_EQUIPMENT_CONFIG = {
    ...
    // Endpoints
    EQUIPMENT_ENDPOINT: '/rest/api/v1/machinery/reg/',
...
class EquipmentAPIService {
    constructor(config = DEFAULT_EQUIPMENT_CONFIG) {
        this.config = config;
    }

    async getEquipmentByRegNumber(regNumber) {
        const url = `${this.config.API_BASE_URL}${this.config.EQUIPMENT_ENDPOINT}${regNumber}`;
