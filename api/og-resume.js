...
module.exports = async (req, res) => {
  try {
    ...
    if (!regNumber) {
      return res.status(400).send('Missing resume ID');
    }
    
    // Fetch resume data
    const apiUrl = `${API_BASE_URL}/rest/api/v1/cv/reg/${regNumber}`;
    const resume = await fetchWithTimeout(apiUrl);
    
    // Generate meta tags
    const title = resume?.fullName 
      ? `Stroyka.kz - Резюме: ${resume.fullName}` 
      : 'Stroyka.kz - Резюме специалиста';
    
    const description = resume 
      ? `${resume.fullName || 'Специалист'}${resume.professions?.[0]?.name ? ` - ${resume.professions[0].name}` : ''}. ${resume.workExperience ? `Опыт: ${resume.workExperience} лет. ` : ''}${resume.city?.name ? `${resume.city.name}. ` : ''}Просмотрите полное резюме в приложении Stroyka.kz.`
      : 'Просмотрите резюме специалиста строительной отрасли в приложении Stroyka.kz';
    
    const url = `${SITE_URL}/resume/${regNumber}`;
    
    // Read the HTML template
    const htmlPath = path.join(process.cwd(), 'resume', 'resume.html');
