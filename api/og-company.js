...
module.exports = async (req, res) => {
  try {
    ...
    // Fetch company data
    const apiUrl = `${API_BASE_URL}/rest/api/v1/analytics/company/${id}`;
    const company = await fetchWithTimeout(apiUrl);
    
    // Generate meta tags
    const title = company?.name 
      ? `Stroyka.kz - ${company.name}` 
      : 'Stroyka.kz - Профиль компании';
    
    const description = company 
      ? `${company.name || 'Компания'}${company.city?.name ? ` - ${company.city.name}` : ''}. ${company.completedProjectsCount ? `${company.completedProjectsCount}+ проектов. ` : ''}${company.description ? company.description.substring(0, 100) : 'Просмотрите профиль в приложении Stroyka.kz'}...`
      : 'Просмотрите профиль строительной компании в приложении Stroyka.kz';
    
    const url = `${SITE_URL}/company/${id}`;
    
    // Read the HTML template
    const htmlPath = path.join(process.cwd(), 'company', 'company.html');
