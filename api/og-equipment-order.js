...
module.exports = async (req, res) => {
  try {
    ...
    if (!regNumber) {
      return res.status(400).send('Missing equipment order ID');
    }
    
    // Fetch equipment order data
    const apiUrl = `${API_BASE_URL}/rest/api/v1/order/special-machinery/reg/${regNumber}`;
    const order = await fetchWithTimeout(apiUrl);
    
    // Generate meta tags
    const title = order?.name 
      ? `Stroyka.kz - Аренда: ${order.name}` 
      : 'Stroyka.kz - Заказ спецтехники';
    
    const description = order 
      ? `${order.name || 'Спецтехника'}${order.orderAmount ? ` - ${order.orderAmount}` : ''}. ${order.address?.name ? `${order.address.name}. ` : ''}${order.plannedDate ? `Срок: ${order.plannedDate}. ` : ''}Просмотрите заказ в приложении Stroyka.kz.`
      : 'Просмотрите заказ на аренду спецтехники в приложении Stroyka.kz';
    
    const url = `${SITE_URL}/equipment-order/${regNumber}`;
    
    // Read the HTML template
    const htmlPath = path.join(process.cwd(), 'equipment-order', 'equipment-order.html');
